...
export const deepseekProvider: ModelProvider = {
  ...
  async diagnose(input) {
    ...
    if (!apiKey) {
      throw new Error("缺少 DEEPSEEK_API_KEY。请在 Vercel 环境变量中配置，或改用 AI_PROVIDER=mock。");
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(`DeepSeek API 调用失败：${response.status} ${detail.slice(0, 240)}`);
    }
    ...
    if (!content) {
      throw new Error("DeepSeek API 没有返回可解析的学习报告。");
    }
